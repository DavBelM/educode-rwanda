...
export function OutputConsole({ output, previewSrc, isRunning, language }: OutputConsoleProps) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--ec-bg)', borderLeft: '1px solid var(--ec-b1)', borderTop: '1px solid var(--ec-b1)' }}>
      {/* Tab bar */}
      <div className="flex items-center shrink-0" style={{ borderBottom: '1px solid var(--ec-b1)', background: 'var(--ec-surface)' }}>
        <button
          ...
          style={{
            ...
            color: tab === 'console' ? '#0ea5e9' : 'var(--ec-text-6)',
          }}
        >
          <Terminal size={14} />
        ...
        <button
          ...
          style={{
            ...
            color: tab === 'preview' ? '#f97316' : 'var(--ec-text-6)',
          }}
        >
          <Monitor size={14} />
        ...
        <div className="ml-auto flex items-center gap-2 pr-3">
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: isRunning ? '#10b981' : 'var(--ec-text-7)' }} />
          {isRunning && (
            <span className="text-xs" style={{ fontFamily: 'Inter, sans-serif', color: 'var(--ec-text-6)' }}>
          ...
          {isRunning && !output ? (
            <div className="flex flex-col items-center justify-center h-full gap-4">
              <div className="flex items-center gap-2">
                ...
                <p className="font-medium" style={{ fontFamily: 'Inter, sans-serif', color: 'var(--ec-text-4)' }}>
            ...
            <pre
              ...
              style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '14px', lineHeight: '1.6', color: 'var(--ec-text-2)' }}
            >
              {output}
            </pre>
          ) : (
            <div className="flex items-center justify-center h-full">
              <p className="text-center text-sm" style={{ fontFamily: 'Inter, sans-serif', color: 'var(--ec-text-7)' }}>
                {isKinyarwanda
      ...
      {tab === 'preview' && (
        <div className="flex-1 overflow-hidden" style={{ background: 'var(--ec-surface-2)' }}>
          {previewSrc ? (
            <iframe
              srcDoc={previewSrc}
